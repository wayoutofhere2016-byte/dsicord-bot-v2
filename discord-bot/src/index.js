...
const { Client, GatewayIntentBits, Collection, Partials, EmbedBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, REST, Routes } = require('discord.js');
...
(async () => {
  try {
    ...
    const commandData = commandFiles.map(f => require(path.join(__dirname, 'commands', f)).data.toJSON());
